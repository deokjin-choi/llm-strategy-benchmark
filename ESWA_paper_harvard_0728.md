...
Human strategic judgments are susceptible to cognitive shortcuts that bypass deliberative reasoning (Kahneman, 2011). The halo effect occurs when a salient positive attribute—such as a firm's reputation—biases overall evaluation (Thorndike, 1920). Anchoring describes the tendency for an initial cue to shape subsequent judgments, even when that cue carries no objective decision-relevant information (Tversky & Kahneman, 1974). More broadly, Kahneman (2011) distinguishes fast, intuitive System 1 thinking from slow, analytical System 2 reasoning, with framing effects arising when System 1 dominates. Parallel work on reasoning LLMs has begun to treat deliberate “slow thinking” as an engineering target analogous to System 2 (Pan et al., 2026), underscoring that dual-process concerns are no longer confined to human judgment.

Emerging evidence suggests that LLM decision agents are likewise vulnerable to context-dependent distortion. He et al. (2026) show that next-token generation under ambiguous context can erode *belief consistency*—the alignment of subsequent choices with an agent’s stated cognition, position, or goal—and propose attribution-based deliberation to realign decisions with those beliefs. Such findings imply that high-status firm-identity cues or selective opportunity-versus-constraint emphasis could likewise trigger heuristic-like reallocations of strategic preference—and reshape the narratives used to justify them—rather than leave the underlying dilemma neutrally evaluated.

What remains missing is a closed-menu characterization of those effects in R&D strategy. Belief-consistency work does not hold a fixed strategic archetype set constant while crossing semantic context with firm-identity framing, nor ask whether matched-choice rationales still diverge when the selected strategy label is unchanged. This study addresses that gap.
...
For deployment, decoding settings must be documented as audited run conditions alongside prompt templates (Section 6.3, Stage 1). A lower-temperature configuration is not a substitute for multi-context or firm-identity audit, and a higher-temperature configuration does not license single-framing production use. The localized cases above motivate the protocol in Section 6.3: sensitivity is measurable and governable only when audit metrics are evaluated on the specific model, scenario, and temperature intended for production—not when inferred from population averages alone.

**6. Discussion**

**6.1 From Findings to the Audit Problem**

Sections 5.1–5.5 show that, with the underlying R&D dilemma and closed strategy menu held fixed, LLM recommendations and matched-choice rationales still shift under semantic context and firm-identity cues. These results do not primarily ask whether any single recommendation is “correct.” They ask whether an LLM-assisted strategy decision support system (DSS) is stable under legitimate deployment variation in how the same problem is narrated and identified. In organizational settings, context blocks, firm names, and prompt templates are routinely edited by analysts; if those edits move strategy mass and explanatory frames, accuracy-oriented evaluation alone is insufficient. Pre-deployment *decision sensitivity audit* is therefore a first-order requirement for trustworthy use in R&D strategy workflows.

**6.2 Interpreting the Four Sensitivity Axes**

The contribution of this study is operational as well as empirical: the framework converts the results of Sections 5.1–5.5 into four measurable sensitivity axes that organizations can recompute on their own model–prompt stacks. *Context sensitivity* captures how much strategy mass moves when opportunity, constraint, competition, or numerical narratives replace the base dilemma (§5.1–5.2); *firm-identity sensitivity* isolates how brand exposure reallocates choice across archetypes (§5.3); *context–identity moderation* asks whether firm-identity effects are themselves context-dependent (§5.4); and *rationale sensitivity under matched choice* tests whether justifications diverge even when model, scenario, context, and the selected strategy are all held fixed (§5.5). Empirically, each axis captures a different aspect of the model's behavior: context shifts are semantic rather than numerical (opportunity > competition > constraint, with numerical perturbation near-invariant), firm identity reallocates the whole menu rather than nudging a single option, moderation changes the intensity of framing effects without reversing their direction, and rationale divergence persists under identical labels. Each is therefore a distinct audit target that a single accuracy or robustness score would obscure.

Interpretively, the firm-identity and rationale patterns are consistent with halo-style retrieval of high-status innovator associations and anchoring of subsequent reasoning on the named firm (Thorndike, 1920; Tversky & Kahneman, 1974): brand exposure elevates leadership-oriented options and tilts justification vocabulary toward high-agency framing even when the chosen archetype is unchanged. More generally, because the four sensitivity axes above are systematic rather than accidental, this sensitivity is predictable enough to audit—and thus amenable to a decision-sensitivity audit framework before deployment.

**6.3 A Decision Sensitivity Audit Protocol for R&D Deployment**

For organizations preparing to use LLMs as R&D strategy DSS components, the framework implies a reusable pre-deployment protocol rather than ad hoc prompt debugging. The protocol profiles *sensitivity*—how recommendations and rationales shift under legitimate prompt variation—not whether any single output is correct. Fig. 15 summarizes the workflow in four stages: freeze the decision object, apply matched perturbations, measure the four sensitivity axes from Section 6.2, and gate deployment with human review.

![Decision_sensitivity_audit_framework](final_results/plots/eval_decision_sensitivity_audit_framework.png)  
Fig. 15\. Scenario-based decision sensitivity audit framework for LLM-based R&D strategic decision support. Stage 1 freezes the dilemma and strategy menu; Stage 2 applies matched firm-identity and multi-context perturbations; Stage 3 measures the four sensitivity axes; Stage 4 applies a human-in-the-loop gate before deployment.

*Stage 1: Freeze the decision object.* Hold the core problem statement and the closed seven-archetype menu fixed, with alphabetically rotated option labels to mitigate order bias and a fixed JSON output schema (one strategy + brief rationale). Record model identity, temperature, and sampling settings as audited run conditions. If decision contexts differ materially from automotive/EV history, validate the menu with domain experts before freezing.

*Stage 2: Apply matched perturbations.* Keeping Stage 1 fixed, vary only the audit factors of interest. For firm identity, run matched Generic vs. Specific prompts under identical model, temperature, scenario, and context load. For semantic context, evaluate at least base, opportunity (*opp\_focus*), competition (*competitive\_dynamics*), and constraint (*count\_fact*), plus *randomized\_numbers* when quantitative inputs matter operationally. The result is a crossed grid of firm-identity × context conditions from which strategy choices and rationales are collected under repeated sampling.

*Stage 3: Measure the four sensitivity axes.* From the Stage 2 grid, compute the Section 6.2 metrics and record them in a Decision Sensitivity Audit Report: (i) context sensitivity via TVD from base (with Spearman rank correlation as a supplementary rank-stability diagnostic); (ii) firm-identity sensitivity via Δ*p* = *p*(Specific) − *p*(Generic) per archetype; (iii) context–identity moderation via difference-in-differences interactions, Δ*p*<sub>interaction</sub> = Δ*p*(variant) − Δ*p*(base); and (iv) rationale sensitivity via RDS on matched-choice pairs, interpreted relative to repeat-noise and cross-strategy ceiling benchmarks (Section 5.5). Flag qualitative patterns that matter for deployment even before numeric triage: opportunity TVD substantially exceeding constraint TVD (optimism over-reaction), near-zero numerical TVD (semantic rather than magnitude-driven shift), brand sign reversal, FDR-significant interactions concentrated in particular narratives, and archetype-specific rationale divergence under identical labels. Packaging these diagnostics as an interpretable report—not merely as back-end scores—is essential: in human–AI decision systems, how uncertainty and related cues are delivered shapes trust and decision efficiency (Sheng et al., 2025).

*Stage 4: Gate with human review.* Human reviewers use the Stage 3 report to decide whether the configuration may proceed, requires human-in-the-loop pilot use, needs prompt or architecture revision and re-audit, or should be withheld pending governance review. This gate follows collaborative human–AI decision frameworks that move among automated use, deferred human review, and joint exploration when uncertainty is high rather than assuming a single always-available expert can absorb every ambiguous case (Tariq et al., 2025). Reliability likewise improves when LLM assessments are reconciled with domain experts through structured collaboration, instead of being accepted in isolation (Jin et al., 2026). Reviewers should explicitly document downside blindness, optimism over-reaction, brand sign reversal, rationale divergence under matched choice, and context-dependent rank reordering. Triage thresholds—if used—should be set to the organization's risk tolerance and re-estimated on the target scenario set (Section 7.1), not treated as universal constants. Re-audit after model or prompt updates and on a scheduled cadence (e.g., quarterly).
...
*Debiasing intervention studies.* Can the observed framing sensitivities be reduced through prompt engineering or model configuration? Promising interventions include: (i) chain-of-thought prompting requiring explicit numerical reasoning, (ii) explicit counter-framing instructions ("Please ignore brand associations and focus on the following facts..."), (iii) ensemble methods aggregating predictions across multiple context variants, and (iv) fine-tuning on balanced examples of optimistic and constraint framing. A useful precedent is work that uses generative models to surface counter-arguments after summarizing a discussion, thereby prompting users' critical thinking rather than consolidating a single narrative (Li et al., 2025); analogous counter-framing probes could be embedded in strategy prompts and evaluated for reductions in context and firm-identity sensitivity. Intervention studies testing these approaches would provide practical tools for immediate deployment.
...
He, G., Zhang, M., Su, T., Ma, L. and Zhu, X. (2026). Enhancing belief consistency of large language model agents in decision-making process based on attribution theory. Expert Systems with Applications, 297, 129273.
...
Jin, W., Gai, T., Fang, J., Fujita, H. and Wu, J. (2026). A group experts–LLMs collaborative decision making method to improve reliability in FMEA risk evaluation. Advanced Engineering Informatics, 71, 104304.
...
Li, S., Han, L. and Demartini, G. (2025). Provoking critical thinking: using counter-arguments in online discussion summarisation. Information Processing & Management, 62(6), 104258.
...
Pan, Q., Ji, W., Ding, Y., Li, J., Chen, S., Wang, J., Zhou, J., Chen, Q., Zhang, M., Wu, Y. and He, L. (2026). A survey of slow thinking-based reasoning LLMs using reinforcement learning and test-time scaling law. Information Processing & Management, 63(2, Part A), 104394.
...
Sheng, L., Chang, F., Sun, Q., Wangzha, D. and Gu, Z. (2025). Uncertainty reports as explainable AI: a cognitive-adaptive framework for human-AI decision systems in context tasks. Advanced Engineering Informatics, 68, 103634.
...
Tariq, S., Baruwal Chhetri, M., Nepal, S. and Paris, C. (2025). A2C: a modular multi-stage collaborative decision framework for human–AI teams. Expert Systems with Applications, 282, 127318.
