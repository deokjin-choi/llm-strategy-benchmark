...
For organizations preparing to use LLMs as R&D strategy DSS components, the framework implies a reusable pre-deployment protocol rather than ad hoc prompt debugging. Fig. 15 summarizes the protocol as an end-to-end audit workflow: freeze the dilemma and strategy menu, apply matched firm-identity and multi-context perturbations, measure the four sensitivity axes, and gate deployment with human review.
...
1. *Freeze the dilemma and the strategy menu.* Hold the core problem statement and the closed set of archetypes fixed so that observed shifts can be attributed to framing rather than to a changing choice set.

2. *Run matched Generic vs. Specific prompts.* Under identical model, temperature, scenario, and context load, compare strategy distributions and report Δp with uncertainty and multiple-testing control. Large reallocations toward identity-congruent archetypes (e.g., leadership under a high-profile innovator name) are a firm-identity sensitivity signal.

3. *Stress-test semantic context variants.* At minimum, evaluate base, opportunity, competition, and constraint narratives—plus a numerical-only control if quantitative inputs matter operationally. Record JSD/rank shifts from base and inspect whether downside information is discounted relative to upside narratives.

4. *Test moderation, not only main effects.* Recompute Δp within each context and inspect interactions versus a designated baseline. If intensity changes materially under constraint or competition while signs remain stable, document multi-context identity audits as mandatory rather than optional.

5. *Audit rationales under matched choices.* For cells in which Generic and Specific runs select the same archetype, inspect keyword polarity and/or RDS against noise and ceiling references. Identical labels with systematically different managerial narratives still fail a decision-support transparency criterion.

6. *Gate with human review and documentation.* Before production use, require (i) Generic–Specific comparison report, (ii) multi-context stress summary, and (iii) rationale-consistency note for high-stakes cells. Human reviewers should explicitly flag missing risk factors when constraint contexts fail to move recommendations, and should treat decoding settings (temperature, sampling) as audited run conditions rather than invisible defaults.

The goal of this protocol is not to eliminate sensitivity—exploratory scenario work may productively exploit it—but to make context and firm-identity effects measurable, reportable, and governable before LLM outputs enter R&D decision processes (Fig. 15).
